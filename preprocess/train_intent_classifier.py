...
from build_intent_input_format import Input_intent_feature
...
def train(cfg, model: BertForSequenceClassification, train_dataset: TensorDataset, validation_dataset: TensorDataset):
    ...
    for epo in range(1, cfg.num_train_epochs + 1):
        ...
        record_txt.writelines(f"epoch: {epo}\t\ttrain loss: {avg_train_loss:.4f}\t\tdev loss: {dev_loss:.4f}\t\tdev acc: {dev_acc:.4f}\n")
...
def validation(cfg, model: BertForSequenceClassification, val_dataloader: DataLoader):
    ...
    label_list = ['questioning', 'acknowleding', 'consoling', 'agreeing', 'encouraging', 'sympathizing', 'suggesting', 'wishing', 'neutral']
...
def load_cache_examples(cfg, data_type):
    file_path = os.path.join(cfg.cache_dir, f"cache_intent_{data_type}_seq{cfg.max_length}")
    print(f"load cache from {file_path}")
    features = torch.load(file_path)

    input_ids = torch.Tensor([f.input_ids for f in features]).long()
    input_masks = torch.Tensor([f.attention_masks for f in features]).long()
...
def main(args):
    ...
    config = BertConfig.from_pretrained(args.bert_path, num_labels=label_num)
