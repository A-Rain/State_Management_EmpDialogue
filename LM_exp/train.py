...
from models.modeling_Emo_Intent_Trans import IntentEmo_Trans_GPT
from models.modeling_emotion_intent_classifier import emotion_intent_classifier
from models.pytorch_transformers import WarmupLinearSchedule, AdamW
from models.pytorch_transformers import GPT2Tokenizer, GPT2Config, BertConfig
from preprocess import Input_feature
...
def load_cache_examples(cfg, data_type):
    ...
    file_path = os.path.join(cfg.cache_dir, f"cache_LM_{data_type}_seq{cfg.max_seq_length}")
    features = torch.load(file_path)

    input_ids_NLU = torch.Tensor([f.input_ids_NLU for f in features]).long()
    input_masks_NLU = torch.Tensor([f.input_masks_NLU for f in features]).long()
...
def get_optimized_paramters(cfg, model: torch.nn.Module):
...
def calc_loss_NLU(cfg, current_logits: torch.Tensor, future_logits: torch.Tensor, intent_logits: torch.Tensor, current_gold_label: torch.Tensor, future_gold_logits: torch.Tensor, intent_gold_label: torch.Tensor):
    ...
    # loss_fut = loss_func_KL(F.log_softmax(future_logits / cfg.emo_tau, dim=-1), F.softmax(future_gold_logits / cfg.emo_tau, dim=-1))
    # loss_fut = loss_fut * (cfg.emo_tau) ** 2
...
def pretrain_classifier(cfg, model: emotion_intent_classifier, train_loader: DataLoader, valid_loader: DataLoader):
    steps_per_batch = len(train_loader)
    t_total = steps_per_batch * cfg.pretrain_NLU_epochs
    optimizer_grouped_parameters = get_optimized_paramters(cfg, model)
    optimizer = AdamW(optimizer_grouped_parameters, lr=cfg.pretrain_NLU_lr, eps=cfg.adam_epsilon)
    ...
    for epo in range(1, cfg.pretrain_NLU_epochs + 1):
        ...
        for batch in tqdm_bar:
            ...
            tqdm_bar.desc = f"pretrain_cls|epoch:[{epo}/{cfg.pretrain_NLG_epochs}],step:[{step}/{steps_per_batch}],avg_loss:{avg_train_loss / step:.4f}"
    _ = validation_NLU(cfg, model, valid_loader)
    return model


def pretrain_lm(cfg, model: IntentEmo_Trans_GPT, train_loader: DataLoader):
    steps_per_batch = len(train_loader)
    t_total = steps_per_batch * cfg.pretrain_NLG_epochs
    optimizer_grouped_parameters = get_optimized_paramters(cfg, model)
    optimizer = AdamW(optimizer_grouped_parameters, lr=cfg.pretrain_NLG_lr, eps=cfg.adam_epsilon)
    scheduler = WarmupLinearSchedule(optimizer, warmup_steps=0, t_total=t_total)
...
def train(cfg, model_NLU: emotion_intent_classifier, model_NLG: IntentEmo_Trans_GPT, 
    ...
    optimizer_grouped_parameters_NLG = get_optimized_paramters(cfg, model_NLG)
    optimizer_NLG = AdamW(optimizer_grouped_parameters_NLG, lr=cfg.lr_NLG, eps=cfg.adam_epsilon)
    ...
    optimizer_grouped_parameters_NLU = get_optimized_paramters(cfg, model_NLU)
    optimizer_NLU = AdamW(optimizer_grouped_parameters_NLU, lr=cfg.lr_NLU, eps=cfg.adam_epsilon)
...
def validation_NLU(cfg, model: emotion_intent_classifier, val_dataloader: DataLoader, is_eval=True):
    dev_loss, count = 0, 0
...
def validation_NLG(cfg, model_NLG: IntentEmo_Trans_GPT, model_NLU: emotion_intent_classifier, val_dataloader: DataLoader):
    model_NLG.eval()
    model_NLU.eval()
    dev_loss, count = 0, 0
...
def sample_sequence(cfg, model: IntentEmo_Trans_GPT, tokenizer: GPT2Tokenizer, context_id: torch.Tensor, type_id: torch.Tensor, 
...
def evaluation(cfg, tokenizer: GPT2Tokenizer, model_NLU: emotion_intent_classifier, model_NLG: IntentEmo_Trans_GPT, test_dataset: List[Dict], test_loader_NLU: DataLoader,
...
def main(args):
    ...
    print("load bert_config ...")
    config_NLU = BertConfig.from_pretrained(args.bert_path)
    setattr(config_NLU, 'num_labels_emo', 7)
    setattr(config_NLU, 'num_labels_intent', 9)
    ...
    model_NLU = emotion_intent_classifier.from_pretrained(args.bert_path, config=config_NLU, emo_trans_matrix=emo_trans_mat, 
                                                          emo_intent_trans_matrix=intent_trans_mat, planning=args.planning)
    ...
    args.special_id_list = get_special_token_ids(args, tokenizer)
    print("load gpt2_config ...")
    config_NLG = GPT2Config.from_pretrained(args.gpt_path)
    model_NLG = IntentEmo_Trans_GPT.from_pretrained(args.gpt_path, config=config_NLG)
    model_NLG.resize_token_embeddings(len(tokenizer))
    ...
    if args.do_train:
        ...
        if args.pretrain_NLG_epochs != 0:
            ...
            if not os.path.exists(pt_NLG_path):
                model_NLG = pretrain_lm(args, model_NLG, train_loader_NLG)
    ...
    if args.do_eval:
        ...
        with open(os.path.join(args.cache_dir, f"decoding_LM_seq{args.max_seq_length}.json")) as f:
            test_json = json.load(f)
        test_loader_NLG, test_loader_NLU = load_cache_examples(args, 'test')

        args.ref_file = os.path.join(args.data_dir, "ref.txt")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--lr_NLU", type=float, default=3e-5)
    parser.add_argument("--planning", type=str, default="cat")
    parser.add_argument("--pretrain_NLU_lr", type=float, default=2e-5)
    parser.add_argument("--pretrain_NLU_epochs", type=int, default=0)
    parser.add_argument("--alpha", type=float, default=0.6)
    parser.add_argument("--beta", type=float, default=0.7)

    # response generator setting
    parser.add_argument("--train_batch_size_NLG", type=int, default=16)
    parser.add_argument("--lr_NLG", type=float, default=8e-5)
    parser.add_argument("--pretrain_NLG_lr", type=float, default=3e-5)
    parser.add_argument("--pretrain_NLG_epochs", type=int, default=1)

    parser.add_argument("--eval_batch_size", type=int, default=16)

    # training epochs
    parser.add_argument("--alternate_num_epochs", type=int, default=5)
    ...
    parser.add_argument("--sampling_tau", type=float, default=1)
    ...
    parser.add_argument("--sche_sig_k", type=int, default=1500)
